...
namespace Assets.MyFolder._01.Script._02.Object.Object_Pooling
{
    ...
    {
        private static PipeObjectPool _instance;
        public static PipeObjectPool Instance
        {
            get
            {
                if (!_instance)
                {
                    _instance = FindFirstObjectByType<PipeObjectPool>();
                    if (!_instance)
                    {
                        GameObject obj = new GameObject("PipeObjectPool");
                        _instance = obj.AddComponent<PipeObjectPool>();
                    }
                }
                return _instance;
            }
